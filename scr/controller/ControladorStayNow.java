...
public class ControladorStayNow {
    ...
    public void cargarDatosTXT() throws ArchivoNoEncontradoException {
        ...
        if (Files.exists(clientesFile)) {
            try (BufferedReader br = Files.newBufferedReader(clientesFile)) {
                ...
                while ((line = br.readLine()) != null && !line.trim().isEmpty()) {
                    ...
                    if (tk.length >= 3) clientes.add(new Cliente(tk[0], tk[1], tk[2]));
                }
            } catch (IOException e) { throw new ArchivoNoEncontradoException("Error cargando clientes", e); }
        }
                ...
                while ((line = br.readLine()) != null && !line.trim().isEmpty()) {
                    ...
                    if (tk.length >= 5) {
                        ...
                        if ("HOTEL".equalsIgnoreCase(tipo) && tk.length >= 7) {
                            ...
                            alojamientos.add(new model.Hotel(codigo, nombre, ubic, precio, estrellas, desayuno));
                            ...
                            alojamientos.add(new model.Apartamento(codigo, nombre, ubic, precio, hab, parqueo));
                        } else if ("CABANA".equalsIgnoreCase(tipo) && tk.length >= 7) {
                            String zona = tk[5];
                            int cap = Integer.parseInt(tk[6]);
                            alojamientos.add(new model.Cabana(codigo, nombre, ubic, precio, zona, cap));
                        }
                    }
                }
            } catch (IOException e) { throw new ArchivoNoEncontradoException("Error cargando alojamientos", e); }
        }
        ...
        if (Files.exists(reservasFile)) {
            ...
            } catch (IOException e) { throw new ArchivoNoEncontradoException("Error cargando reservas", e); }
        }
    }
